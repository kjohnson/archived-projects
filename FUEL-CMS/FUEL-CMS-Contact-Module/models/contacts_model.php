...
class Contacts_model extends Base_module_model {
	...
	public $record_class = 'Contact'; // the name of the record class (if it can't be determined)
	...
	{
		parent::__construct('fuel_contact_contacts', CONTACT_FOLDER); // table name
	}
	 ...
	 * List Contacts
	 *
	 * Returns a list of contacts in 'ID' & 'Display Name' pairs
	 *
	 * @return Object
	 */
	function list_contacts($col = 'precedence', $order = 'desc') {
		return $this->db->select('id, display_name')
						->where('published', 'yes')
						->order_by($col, $order)
						->get($this->table_name)
						->result_array();
...
class Contact_model extends Base_module_record {
