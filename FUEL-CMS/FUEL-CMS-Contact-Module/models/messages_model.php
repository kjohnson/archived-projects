...
require_once(MODULES_PATH.'/contact/config/contact_constants.php');

class Messages_model extends Base_module_model {

	// read more about models in the user guide to get a list of all properties. Below is a subset of the most common:

	public $record_class = 'Message'; // the name of the record class (if it can't be determined)
	...
	{
		parent::__construct('fuel_contact_messages', CONTACT_FOLDER); // table name
	}

	function list_items($limit = NULL, $offset = NULL, $col = 'submitted', $order = 'desc', $just_count = FALSE)
	{
	...
	{
		parent::on_before_insert($values);

		if (empty($values['submitted']))
		{
			$values['submitted'] = date('Y-m-d H:i:s', time());
		}
...
class Message_model extends Base_module_record {
